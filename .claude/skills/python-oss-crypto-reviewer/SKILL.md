...
- **For any fix that touches what the scanner observes or reports about a live TLS endpoint** (protocol/cipher/certificate detection, findings, CBOM output): cross-check against `testssl.sh` (or nmap's `ssl-enum-ciphers`/`ssl-cert` scripts) against the same real target, not just the project's own fixtures. testssl.sh reports ~400+ discrete data points per endpoint (protocol versions, full cipher lists per protocol, cert chain/OCSP/CRL/CT, fingerprints) — qureddy's fixture-based tests can't catch a gap in *scope* (e.g. #192: qureddy never checks for legacy TLS 1.0/1.1 exposure at all, discovered only by diffing against testssl.sh's real output, not by any unit test). A patch that changes detection logic can pass every existing fixture test and still silently narrow or misrepresent what's actually true about the target.
...
- **Stream contamination** (#15, revisited #194): log handlers binding `sys.stderr` at function-call time get captured by test runners that mix streams. Production looks fine, tests fail. Fix: bind to the kernel-level fd via `os.dup(2)`. **Caveat found in #194: this fix only covers in-process stream mixing (test runners, `redirect_stderr`), not genuine shell-level `2>&1`.** A real shell redirect calls `dup2()` on the OS file descriptor table *before the process starts* — fd 1 and fd 2 are already the same underlying pipe by the time Python runs, so `os.dup(2)` just returns a second handle to the already-merged destination. There is no "separate original stderr" left to snapshot. Verify any #15-style fix against **real** `2>&1` (`subprocess.run(..., stderr=subprocess.STDOUT)` against the installed console script, or a literal shell pipe) — `CliRunner`-based tests, even a correctly-configured one, structurally cannot exercise this and will give false confidence.
...
- **Click `mix_stderr=True` defaults** (#25, superseded by #189): historically, tests that read `result.stderr` without `mix_stderr=False` crashed with `ValueError: stderr not separately captured`, fixed via `CliRunner(mix_stderr=False)`. **This entry is now stale against current Typer/Click** — `typer.testing.CliRunner` has dropped `mix_stderr` entirely (not deprecated, just gone; confirmed against Typer 0.27/Click 8.1). `result.stdout` and `result.stderr` are always separate now; there is no merge option. Tests written against the old assumption (checking `result.stdout` for text that Click actually writes to stderr) fail with an empty string, not an exception — check `result.stderr` directly instead.
...
- **Vendored-exception drift across a dependency boundary** (#186): a dependency (Typer 0.27+) can silently start raising its own internal fork of another library's (Click's) exception classes — same name, same shape, different class object. `isinstance(exc, click.exceptions.UsageError)` silently stops matching; nothing crashes, the code just falls through to the wrong `except` clause. `pyproject.toml`'s open-ended `"typer>=0.12"` (no upper bound) let this drift in on a routine install. Fix: match by name across the MRO (`any(cls.__name__ == "UsageError" for cls in type(exc).__mro__)`) instead of `isinstance` against one specific package's class; pin dependency ranges that have known vendoring/exception-hierarchy risk.
- **Cross-repo private-API coupling** (#196): a producer repo (qureddy's `cbom.py`) reverse-engineered a consumer repo's (Qurum's) private, underscore-prefixed function (`_linked_libraries`) to match its exact expected graph shape, documented by name in a comment. Works today, but nothing in either repo's CI would catch drift if the consumer refactors that private function — there's no shared schema, versioned contract, or cross-repo integration test. When a fix depends on another BreachSAFE repo's internal behavior, flag the missing contract test as part of the review, not just praise for getting today's shape right.
