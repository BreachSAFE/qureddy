...
def _run_qureddy(*args: str, merge_stderr: bool) -> subprocess.CompletedProcess[str]:
...
def test_machine_output_keeps_stderr_hint_with_separate_streams(
    ...
    """Issue #274 preserved: separate fds still get the actionable stderr hint."""
    result = _run_qureddy(*_TLS_EXIT3_ARGS, "--format", output_format, merge_stderr=False)
    assert result.returncode == 3
