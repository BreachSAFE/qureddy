...
def test_json_logs_suppress_cyclonedx_debug_records() -> None:
    """CycloneDX serializer internals must not leak into verbose scan logs."""
    fake_stderr = io.StringIO()
...
def test_json_logs_wrap_other_third_party_stdlib_records() -> None:
    """Foreign stdlib records must remain one valid JSON object per line."""
    fake_stderr = io.StringIO()
    configure_logging(verbosity=2, json_logs=True, log_stream=fake_stderr)

    logging.getLogger("example_dependency").debug("Processed %s", "record")

    lines = fake_stderr.getvalue().splitlines()
    assert len(lines) == 1
    payload = json.loads(lines[0])
    assert payload["event"] == "Processed record"
