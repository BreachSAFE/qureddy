...
the only place in the codebase (alongside capability.py) that calls `openssl` via
`subprocess`. Other modules consume typed `OpenSSLDependency` and `ProbeResult` values.

Capability detection (path resolution, version, PQC group support) lives in
capability.py — split out per the file-size gate (coding-rules.md §2.2).
`resolve_openssl_path`, `probe_capability`, `raise_if_unusable`, `HYBRID_GROUP`,
and `CLASSICAL_GROUP` are re-exported here unchanged so existing imports
(scanner.py, tests) keep working without changes.
"""
