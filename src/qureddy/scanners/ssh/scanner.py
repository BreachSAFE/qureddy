...
def build_ssh_failure_result(
    ...
    *,
    ...
    """Build a `ScanResult` for the SSH probe-failure exit-2 path.

    Machine formats emit exactly one parseable document even when the
    probe fails (issue #30) — this mirrors the TLS scanner's
    `build_capability_failure_result`. The probe raises connect-level
    failures `from` the underlying OSError/TimeoutError, so a present
    cause maps to TARGET_CONNECT_FAILED; malformed-response failures
    (raised without a cause) map to PARSE_AMBIGUOUS.

    Args:
        target: The parsed SSH scan target.
        error: The `SSHProbeError` the probe raised.
        cleaned_error: Operator-facing message (errno noise stripped);
            recorded in the evidence notes so the document explains itself.
    """
