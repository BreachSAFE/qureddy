...
def scan_tls(
    ...
    # JSON stdout is a single machine-parsed document. The #15 fd-snapshot
    # fix only protects against in-process stream rebinding (CliRunner,
    # etc.); it cannot protect real shell `2>&1` (the OS has already
    # merged fd 1/2 before Python starts). A WARNING+ log line during the
    # scan silently corrupts that document for any real `| jq`-style
    # consumer. Default to quiet in this format so the common case is
    # safe by default; an explicit -v/-vv/-vvv still wins, since that's
    # the user asking for diagnostics and accepting they must keep
    # stdout/stderr genuinely separate (not `2>&1`) to still get clean JSON.
    effective_quiet = quiet or (output_format is OutputFormat.JSON and verbose == 0)
