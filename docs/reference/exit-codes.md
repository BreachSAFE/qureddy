...
| **3** | `EXIT_LOCAL_DEPENDENCY` | Required local executable is missing or unsupported | TLS requires OpenSSL 3.5.7 or newer within the 3.5.x LTS series. IKE requires a usable stock `ike-scan` executable. |
...
case $? in
  ...
  2)
    ...
    ;;
  3)
    echo "Runner is misconfigured (OpenSSL missing, broken, below 3.5.7, outside 3.5.x, or lacks the required group)"
    exit 1
    ;;
  4)
