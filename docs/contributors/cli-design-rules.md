...
Both `--version` and the short form `-V` (capital, per GNU §4.7) print the version banner and nothing else. The banner must be machine-parseable: a single line, predictable shape, version string at a fixed offset. Implementation reference: PR [#55](https://github.com/breachsafe/qureddy/pull/55) and [ADR 0003](adr/0003-cli-help-rewrite.md).
...
The contract that makes `qureddy scan tls TARGET --format json | jq` work. Logs go to stderr. Scan output goes to stdout. Crashes go to stderr. **This rule is the most-broken in QuReddy's history** — see issues [#15](https://github.com/breachsafe/qureddy/issues/15), [#22](https://github.com/breachsafe/qureddy/issues/22), [#24](https://github.com/breachsafe/qureddy/issues/24) for the canonical cautionary tale where a `print()` mid-scan and a `CliRunner` fixture conspired to leak logs into stdout. Citation: [clig.dev "Output"](https://clig.dev/#output), POSIX file descriptor convention.
...
QuReddy uses **0 / 2 / 3 / 4 / 70**, NOT BSD `sysexits.h`. The full surface lives in [`docs/reference/exit-codes.md`](../reference/exit-codes.md). When a code is added, ADR + maintainer sign-off are required (see [ADR 0001](adr/0001-trace-and-verbosity.md) for the canonical decision shape, and [#12](https://github.com/breachsafe/qureddy/issues/12) for the addition of code 70). Reviewing this rule's violation is one of the harder catches because exit-code drift is invisible until a CI script `if $? -eq 2` script breaks. The PR template's exit-code checklist exists for exactly this reason.

**Rule 1.12 — Helpful errors on common typos.**
When the user types `--v`, `--vv`, `--verbos`, or applies `--version` to a subcommand instead of root, the error message names the right form. Click's default "no such option" message is too cryptic. PR [#80](https://github.com/breachsafe/qureddy/pull/80) and PR [#65](https://github.com/breachsafe/qureddy/pull/65) are the implementations.
...
Click's default for `qureddy scan tls --version` is "no such option: --version." That is true (the option is on the root, not the subcommand) but unhelpful. PR [#65](https://github.com/breachsafe/qureddy/pull/65) detects the misplacement and prints "did you mean `qureddy --version`?" before exiting 4.

**Rule 3.6 — Verbosity is a counter, stacked POSIX-style.**
`-v` (INFO), `-vv` (DEBUG), `-vvv` (DEBUG + traceability panel). The `--verbose` long form takes no count argument. Typo handling for `--v`, `--vv`, `--vvv`, `--verbos` is implemented per Rule 1.12 in PR [#80](https://github.com/breachsafe/qureddy/pull/80) (closes [#74](https://github.com/breachsafe/qureddy/issues/74)).
...
- **Input validation gaps.** Tracked in issues (e.g. [#13](https://github.com/breachsafe/qureddy/issues/13)), not rules.
- **`--help` snapshot tests.** Tracked in [#88](https://github.com/breachsafe/qureddy/issues/88).
- **Exit-code contract tests.** Tracked in [#89](https://github.com/breachsafe/qureddy/issues/89).
