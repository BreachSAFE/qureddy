...
| Integration tests | 3 | `pytest tests/test_openssl_probe.py` | needs supported OpenSSL 3.5.x LTS on runner |
...
CI runs on the matrix: **ubuntu-latest × macos-latest × windows-latest × Python 3.14**. All three platforms must pass for both tiers. Supported OpenSSL 3.5.x LTS is installed per-platform during CI setup.
