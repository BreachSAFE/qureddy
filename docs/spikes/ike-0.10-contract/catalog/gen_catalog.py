...
SPEC = {
    ...
    ("2", "transform_type"):("ikev2-parameters.xml","ikev2-parameters-3",  "transform_type_registry"),
...
SCHEDULED = {
    ("2","key_exchange",14),("2","key_exchange",19),("2","key_exchange",31),
    ("2","encryption",12),("2","encryption",20),("2","prf",5),("2","integrity",12),
    ("1","encryption",7),("1","hash",2),("1","key_exchange",14),("1","authentication",1),
}
...
def parse(path: Path, reg_id: str) -> list[dict]:
    ...
    for rec in re.findall(r'<record.*?</record>', m.group(0), re.S):
        ...
        des = (re.search(r'<description>(.*?)</description>', rec, re.S)
               or re.search(r'<name>(.*?)</name>', rec, re.S))
...
def classify(name: str, status: str | None) -> str:
    ...
    if n in ("unassigned",):              return "unassigned"
    if "reserved for private use" in n:   return "private_use"
    if n.startswith("reserved"):          return "reserved"
    return "current"

def main() -> int:
    files = {f: (CORPUS / f) for f in {v[0] for v in SPEC.values()}}
    prov = {f: {"sha256": sha256(p), "updated": updated(p.read_text(encoding='utf-8'))}
    ...
    for (ver, role), (fname, reg_id, wire) in sorted(SPEC.items()):
        for r in parse(files[fname], reg_id):
            ...
            wid = int(v)
            key = (ver, role, wid)
            sched = ("scheduled_0_10" if key in SCHEDULED
                     else "post_mvp_0_14_555" if key in POST_MVP
                     else "catalog_only_never_scheduled" if key in NEVER_SCHEDULED
                     else "catalog_only")
            entries.append({
                "ike_version": ver, "role": role, "wire_location": wire,
                "wire_id": wid, "name": r["name"],
