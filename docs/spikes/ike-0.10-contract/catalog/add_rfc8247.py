#!/usr/bin/env python3
"""Add the RFC 8247 requirement axis. IANA status and RFC 8247 requirement
are INDEPENDENT: ENCR_NULL is IANA-current and RFC 8247 MUST NOT; ENCR_3DES
is IANA-current and RFC 8247 MAY. Conflating them manufactures findings."""
import json, re, sys
from pathlib import Path

RFC = Path("/Users/paul/claude/breachsafe-standards/standards/rfc/rfc8247/rfc8247.txt")
ROW = re.compile(r'^\s*\|\s*([A-Z][A-Z0-9_]+)\s*\|\s*(MUST NOT|SHOULD NOT|MUST-?|SHOULD\+?|SHOULD|MAY|MUST)\s*\|')
...
def main() -> int:
    ...
    doc["rfc8247_axis"] = {
        ...
        "rows_parsed": len(reqs), "entries_matched": hit, "entries_unmatched": miss,
    }
    ...
    for n in ("ENCR_NULL","ENCR_3DES","ENCR_DES","PRF_HMAC_MD5"):
        r = reqs.get(n)
        print(f"    {n:20} {r['requirement'] if r else 'not in tables':10} {('rfc8247:'+str(r['line'])) if r else ''}")
    return 0
