...
Google's frontends support hybrid post-quantum key exchange. Scan one:

```bash
qureddy scan tls www.google.com
```

The tool runs two TLS handshakes (one hybrid, one classical), parses the results, and prints a verdict panel. You should see something like:
...
The big banner at the top is the at-a-glance verdict. Green border + "READY" means Google's TLS endpoint negotiates a hybrid post-quantum key exchange when the client offers it.
...
The server negotiated `X25519MLKEM768` — a hybrid of X25519 (classical) and ML-KEM-768 (post-quantum). "Transitional" because the certificate chain is still classical (RSA/ECDSA), so a future quantum attacker could still impersonate Google by forging a cert. Hybrid key exchange protects the *session secret* against harvest-now-decrypt-later attacks.

**The two probes** (hybrid + classical)
...
**The findings count** (2)
One finding per probe outcome that matched a policy rule. Both findings are visible in the JSON output (Step 5).
