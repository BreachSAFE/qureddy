...
| `version_unique` | Met | `pyproject.toml` contains the package version; each release bumps it (`scripts/bump_version.py`). |
| `version_semver` | Met | Semantic versioning on the `0.2.x` line. |
| `version_tags` | Met | Each release has a matching `v<version>` tag. |
...
OpenSSL 3.5.x LTS binary for TLS observation and reads the server's cleartext
SSH KEXINIT offer. The `crypto_*` criteria are answered on that basis.

| id | status | evidence |
|---|---|---|
| `crypto_published` | Met | No custom crypto; relies on OpenSSL 3.5.x LTS (published, standard). |
