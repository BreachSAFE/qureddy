...
- OpenSSL 3.5 or newer for `scan tls` only
...
If the version is older than 3.5 or the group list does not contain
...
For TLS scans, install a trusted OpenSSL 3.5 or newer Windows build. QuReddy
...
The selected binary must report OpenSSL 3.5 or newer and list
`X25519MLKEM768`.
...
The release metadata requires Python `>=3.12,<3.13`. Point pipx at Python
3.12 explicitly:
