...
3.5.x LTS binary (3.5.7 or newer within the 3.5 series). IKE scanning requires
a separate stock `ike-scan` executable.
...
- OpenSSL 3.5.x LTS, at least 3.5.7, for `scan tls` only
...
If the executable or any explicitly reported linked-library version is not in
the 3.5.x LTS series at 3.5.7 or newer, or the group list does not contain `X25519MLKEM768`, install
a supported OpenSSL 3.5.x LTS vendor build or use the repository's
[checksum-pinned 3.5.x source-build recipe](../../.github/actions/setup-openssl/action.yml)
...
For TLS scans, install a trusted OpenSSL 3.5.x LTS Windows build (3.5.7 or newer
within the 3.5 series). QuReddy
...
The selected binary must report OpenSSL 3.5.7 or newer within the 3.5.x LTS
series and list `X25519MLKEM768`.
